...
<link rel="stylesheet" type="text/css" href="../itek.css">
	...
	<tr>
		...
		if (isset($submit) &&($submit=="Обновить"))
		{
		    echo "Обновлено";
		    to_baze($ddid,$iname,$ikol,$iH,$iW,$iL,$icost);
	            to_vitrina($ivid,$ihead,$iimg,$idesc,$inote);
		    
		    ins_baze($ddid,$nname,$nkol,$nH,$nW,$nL,$ncost,$nvid);
		    ins_vitrina($nhead,$nimg,$ndesc,$nnote,$rid);
		    
		    in_catalog($page,$rid,$zakaz,$filt);
		}
		if (!isset($submit))
		{
		    in_catalog($page,$rid,$zakaz,$filt);
